...
*   **Fixed "undefined" being stored for user after manual timezone update by modifying `frontend/src/components/settings/TimezoneSelector.tsx` to use the full user object returned by the `updateTimezone` API call, ensuring consistency in user data storage.**
